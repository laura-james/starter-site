...
<?php include "includes/connectdb.php" ?>
   ...
   if (isset($_SESSION["isAdmin"]) && $_SESSION["isAdmin"] == 1) { 
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $stmt = $conn->prepare("UPDATE courses SET name = ?, description = ?, date = ?, capacity = ?, category_id= ?  WHERE course_id = ?");
        $stmt->bind_param("sssiii", $name,$description,$date,$capacity,$category_id,$course_id);
